...
public class ManuallyForwardingTab extends Fragment
{
    ...
    private void showPortNotInRange()
    {
        Toast toast = Toast.makeText(getActivity().getApplicationContext(), R.string
                .portRangeMsg1, Toast.LENGTH_SHORT);
        toast.setGravity(Gravity.CENTER, 0, 0);
        toast.show();
    }
    ...
    {
        Toast toast = Toast.makeText(getActivity().getApplicationContext(), "Please fill in " +
                "internal port!", Toast.LENGTH_SHORT);
        toast.setGravity(Gravity.CENTER, 0, 0);
        toast.show();
    }
    ...
    private void showAddBtnDialog()
    {
        ...
        final Spinner spinnerChoosePortType = (Spinner) addPortView.findViewById(R.id
                .portType_spinner);
        ...
        final EditText addInternalEdit = (EditText) addPortView.findViewById(R.id
                .internalPort_edit);
        final EditText addExternalEdit = (EditText) addPortView.findViewById(R.id
                .externalPort_edit);
                        ...
                        else
                        {
                            if(choosedPort.equals("HTTP"))
                            {
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid
                                        (), "http", internalInt);
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid(), "exthttp", externalInt);
                            }
                            else if(choosedPort.equals("RTSP"))
                            {
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid(), "rtsp", internalInt);
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid(), "extrtsp", externalInt);
                            }
                            if(choosedPort.equals("HTTPS"))
                            {
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid(), "https", internalInt);
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid(), "exthttps", externalInt);
                            }
                            else if(choosedPort.equals("FTP"))
                            {
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid(), "ftp", internalInt);
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid(), "extftp", externalInt);
                            }
                            else if(choosedPort.equals("SSH"))
                            {
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid(), "ssh", internalInt);
                                cameraOperation.updateAttributeInt(camera.getIP(), camera.getSsid(), "extssh", externalInt);
                            }
