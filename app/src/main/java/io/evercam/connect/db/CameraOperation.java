...
public class CameraOperation
{
    ...
    public void updateScanCamera(Camera camera, String ssid)
    {
        ...
        db.execSQL("update cameralist set mac=?,vendor=?,lastseen=?,flag=?,active=?  where ip=? " + "and ssid=?", new Object[]{camera.getMAC(), camera.getVendor(), camera.getLastSeen
    ...
    {
        db = database.getWritableDatabase();
        db.execSQL("insert into cameralist(ip,mac,vendor,model,upnp, onvif, bonjour,http,https," +
				"rtsp,ftp, ssh, portforwarded, evercam,exthttp,exthttps, extftp,extrtsp,extssh, " +
				"flag,firstseen,lastseen,username, password, ssid, active)values(?,?,?,?,?,?,?,?," +
				"?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", new Object[]{camera.getIP(), camera.getMAC
				(), camera.getVendor(), camera.getModel(), camera.getUpnp(), camera.getOnvif(),
				camera.getBonjour(), camera.getHttp(), camera.getHttps(), camera.getRtsp(), camera
				.getFtp(), camera.getSsh(), camera.getPortForwarded(), camera.getEvercamConnected
				(), camera.getExthttp(), camera.getExthttps(), camera.getExtftp(), camera
				.getExtrtsp(), camera.getExtssh(), camera.getFlag(), camera.getFirstSeen(), camera
				.getLastSeen(), camera.getUsername(), camera.getPassword(), ssid, camera.getActive
				()});
        db.close();
    }
    ...
    {
        db = database.getWritableDatabase();
        db.execSQL("update cameralist set mac=?,vendor=?,model=?,http=?,bonjour=?,lastseen=?," +
				"active=?  where ip=? and ssid=?", new Object[]{camera.getMAC(), camera.getVendor
				(), camera.getModel(), camera.getHttp(), camera.getBonjour(), camera.getLastSeen()
    ...
    {
        db = database.getWritableDatabase();
        db.execSQL("update cameralist set model=?,http=?,upnp=?,lastseen=?,active=?  where ip=? " +
				"and ssid=?", new Object[]{camera.getModel(), camera.getHttp(), 1, camera
				.getLastSeen(), camera.getActive(), camera.getIP(), ssid});
        db.close();
    }
    ...
    {
        db = database.getWritableDatabase();
        Cursor c = db.rawQuery("select * from cameralist where mac=? and ssid=?", new
				String[]{mac, ssid});
        if(c.moveToFirst())
        {
            return true;
    ...
    {
        db = database.getWritableDatabase();
        db.execSQL("update cameralist set " + attribute + "=?  where ip=? and ssid=?", new
				Object[]{value, ip, ssid});
        db.close();
    }
    ...
    public ArrayList<Camera> selectAllIP(String ssid)
    {
        ...
        Cursor cursor = db.rawQuery("select * from cameralist where ssid=? order by ip", new
				String[]{ssid});
    ...
    public ArrayList<Camera> selectCameraOnly(String ssid)
    {
        ...
        try
        {
            Cursor cursor = db.rawQuery("select * from cameralist where ssid=? and flag=? order by ip", new String[]{ssid, "1"});
            while(cursor.moveToNext())
            {
                String ip = cursor.getString(1);
                Camera camera = getCamera(ip, ssid);
