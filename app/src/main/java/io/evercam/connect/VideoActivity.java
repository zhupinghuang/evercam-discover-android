...
public class VideoActivity extends Activity implements SurfaceHolder.Callback, IVideoPlayer
{
    ...
    private void addUrlIfValid(String url)
    {
        try
        {
            if(url == null || url.trim().length() < 10 || !(url.startsWith(Constants.PREFIX_HTTP)
                    || url.startsWith(Constants.PREFIX_RTSP)))
                return;
    ...
    @Override
    public void setSurfaceSize(int width, int height, int visible_width, int visible_height, int
            sar_num, int sar_den)
    {
    ...
    private void createPlayer(String media)
    {
        ...
        {
            Toast.makeText(this, "Error connecting! " + media + " ::::: " + e.getMessage(), Toast
                    .LENGTH_LONG).show();
        }
        catch(Error e)
        {
    ...
    {
        CustomedDialog.getAlertDialog(this, getString(R.string.msg_unable_to_play), getString(R
                .string.msg_video_not_avaliable), new DialogInterface.OnClickListener()
                {
                    @Override
    ...
    private void initialPageElements()
    {
        ...
        mediaPlayerView.setOnClickListener(new OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                if(end)
                {
                    Toast.makeText(VideoActivity.this, R.string.msg_try_again, Toast
                            .LENGTH_SHORT).show();
                    return;
                }
     ...
     */
    public InputStream getStreamFromUrl(String url, String username, String password) throws
            UnirestException
    {
                ...
                try
                {
                    ...
                    // command
                    {
                        ...
                        if(downloadStartCount - downloadEndCount < 9)
                            tasklive.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new
                                    String[]{imageURL});
        ...
        protected Drawable doInBackground(String... urls)
        {
            ...
            for(String url : urls)
            {
                try
                {
                    ...
                    if(!url.isEmpty())
                    {
                        InputStream stream = getStreamFromUrl(url, camera.getUsername(), camera.getPassword());
